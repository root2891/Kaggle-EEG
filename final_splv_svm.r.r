...
for(patient in patients)
{
  ...
  {
    print(i)
    #index_list<- createDataPartition(targetDataSet[,1], p = .75, list = FALSE)
    trainInput <- splv_result_array[i,1:7,]
    #testInput <- splv_result[i,,-index_list]
    trainInput <- t(trainInput)
    trainTarget <- targetDataSet
    #testTarget <- targetDataSet[-index_list]
    model_list[[i]] <- svm(trainInput, trainTarget, kernel = "radial", type = "nu-classification",scale = TRUE, nu = 0.1)
  }
