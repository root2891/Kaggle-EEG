...
v <- dyn.load("D:/Work Related/Kaggle EEG/dtw_r2.so")
...
calculation_per_combination <- function(inputData.data, noOfRods, frequency)
{
  ...
  write.table(inputData.data, "D:/Work Related/Kaggle EEG/table2.txt", row.names = F, col.names = F)
  outputVal <-1:ncol(combn(noOfRods, 2))
  out <- dtw2(noOfRods,frequency, as.double(outputVal) )
  unlink("C:/Shubham/table2.txt")
...
process_Data <- function(k, patient, filetype)
{
  ...
  dtw_result<-rbind(dtw_result,dtw = calculation_per_combination(inputData.data,noOfRods, frequency), target = targetDataSet)
  return (dtw_result)
}
...
for(patient in patients)
{
  ...
  noOfRods <- 16
  frequency <- 400
    c1<- makePSOCKcluster(2, outfile = "temp.log")
    clusterEvalQ(c1,library("R.matlab"))
    clusterEvalQ(c1,library("e1071"))
    clusterEvalQ(c1,library("combinat"))
    clusterEvalQ(c1,library("dtw"))
    clusterEvalQ(c1,library("parallel"))
    clusterEvalQ(c1, dyn.load("D:/Work Related/Kaggle EEG/dtw_r2.so"))
    clusterExport(c1, varlist =c("get_data", "calculation_per_combination", "get_no_of_files", "dtw2", "noOfSeconds", "path", "process_Data","patient", "filetype", "targetDataSet", "dtw_result"))
    output<- parLapply(c1,1:10, function(x) process_Data(x, patient, filetype) )
    stopCluster(c1)
